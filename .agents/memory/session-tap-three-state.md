...
1. `skipMiniPlayer` → `playSession(s); return;` — reproduce al instante, SOLO aparece el miniplayer, no se navega a ninguna pantalla. La barra visible es `SessionMiniPlayer` (montada en `(tabs)/_layout.tsx`, overlay sobre todas las tabs incl. category/*): se muestra porque `playSession` en PlayerContext emite `sessionMiniPlayerEvents.triggerShow()` cuando `session.skipMiniPlayer` — NO hace falta emitir en cada sitio de tap. Se suprime vía prop `suppressed` cuando hay `activePlaylist` (evita doble barra con PlaylistMiniPlayer). El `MiniPlayer.tsx` morado es OTRO componente (solo panel Mezclador + mezclas/*).
