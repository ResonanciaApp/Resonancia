...
- **Validación de uploads (hardening pendiente)**: `POST /storage/uploads/request-url` valida allowlist de contentType + tamaño y registra metadatos en tabla `uploads`, pero la URL firmada (presigned PUT) NO restringe content-type ni content-length. Un cliente podría pedir la URL con metadatos válidos y luego subir otro MIME/tamaño. Mejora futura: firmar la URL con esas restricciones (coordinar con clientes que ya suben: chat image/audio, avatar, videos) + tests de enforcement.
...
- **Formato de audio (DECISIÓN tomada)**: estándar de distribución = **AAC** (`.m4a`), NO WAV ni MP3 ni Opus. WAV solo para masters de producción. Bitrates: cuencos/gongs/ancestrales y música ambient → AAC 256 VBR; meditaciones guiadas (voz) → AAC 128-192; loops base Naturaleza → AAC 128-160 (cortos, bundle). Premium Hi-Fi futuro → FLAC/WAV descargable opcional. Descartado: Opus (soporte inconsistente en RN/iOS vía expo-av) y HLS (sobre-ingeniería para sesiones de 10-50 min; ya se sirve con progressive download + range requests). Foco: bundlear solo lo imprescindible (loops base), streamear el resto desde Object Storage. Pipeline ffmpeg: `-c:a aac -b:a 256k` o `-q:a` para VBR. Detalle completo en `docs/formato-audio-decision.docx`.
