...
- **Free vs Premium (gating de funcionalidades)**: armar `PremiumContext` con flag `isPremium`, hook `useGate(feature)`, componente `<PremiumLock>` y archivo `config/premium-features.ts` con la lista. Falta definir con el usuario qué entra en cada plan.
