    ...
    <div className="relative w-screen h-screen overflow-hidden font-display" style={{ backgroundColor: "#1B060F", color: "#F4DAD5" }}>
      ...
      <div style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", padding: "8vh 8vw", zIndex: 2 }}>
        ...
        <div style={{ fontSize: "5vw", fontWeight: 700, lineHeight: 1.0, letterSpacing: "-0.03em", marginBottom: "7vh" }}>
          El campo está<br /><span style={{ color: "#D4AF37" }}>abierto.</span>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          {/* Header */}
          <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 2fr 2fr 2fr", gap: "0 2vw", paddingBottom: "1.5vh", borderBottom: "1px solid rgba(212,175,55,0.2)", marginBottom: "0" }}>
            {cols.map((c) => (
              <div key={c} style={{ fontSize: "1.05vw", fontWeight: 600, letterSpacing: "0.15em", color: "rgba(244,218,213,0.35)" }}>{c.toUpperCase()}</div>
            ))}
          </div>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.app} style={{
                ...
                padding: "2vh 0",
                borderBottom: `1px solid ${isUs ? "rgba(212,175,55,0.3)" : "rgba(244,218,213,0.06)"}`,
                backgroundColor: isUs ? "rgba(212,175,55,0.05)" : "transparent",
              }}>
                <div style={{ fontSize: isUs ? "1.7vw" : "1.55vw", fontWeight: isUs ? 700 : 500, color: isUs ? "#D4AF37" : "rgba(244,218,213,0.8)" }}>{r.app}</div>
                {[r.idioma, r.contenido, r.precio, r.comunidad].map((v, j) => (
                  <div key={j} style={{ fontSize: isUs ? "1.5vw" : "1.4vw", fontWeight: isUs ? 600 : 400, color: isUs ? "#F4DAD5" : "rgba(244,218,213,0.55)" }}>{v}</div>
                ))}
              </div>
            );
          })}
