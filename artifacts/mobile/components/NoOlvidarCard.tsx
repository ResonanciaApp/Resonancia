...
function VozCard({
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={styles.topRow}>
        <View style={[styles.badge, { backgroundColor: "#D4AF3722", borderColor: "#D4AF3755" }]}>
          <Feather name="mic" size={9} color="#D4AF37" style={{ marginRight: 3 }} />
          <Text style={[styles.badgeText, { color: "#D4AF37" }]}>Voz Interior</Text>
        </View>
        <Text style={[styles.dateText, { color: colors.mutedForeground }]}>{formatDate(item.createdAt)}</Text>
        <Pressable onPress={confirmRemove} hitSlop={10} style={styles.deleteBtn}>
          <Feather name="trash-2" size={13} color={colors.mutedForeground} />
