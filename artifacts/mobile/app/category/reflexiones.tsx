...
function SearchOverlay({ visible, onClose }: { visible: boolean; onClose:()=>void }) {
  ...
  const results = useMemo(()=>q.length>=2?SESSIONS.filter((s)=>s.categoryId==="reflexiones"&&s.title.toLowerCase().includes(q.toLowerCase())):[],[q]);
  useEffect(()=>{
    ...
    <View style={styles.root}>
      <View style={[styles.header,{height:HERO_HEIGHT+topPad}]}>
        ...
        <View style={styles.headerTopRow}>
          ...
          <Pressable hitSlop={10} style={styles.headerIconBtn} onPress={()=>router.push("/reflexiones-info" as never)}><Feather name="plus" size={24} color="#fff" /></Pressable>
        </View>
        <View style={styles.heroTitleArea}>
          <View style={styles.heroTitleRow}>
            <Text style={styles.heroTitle}>Reflexiones</Text>
...
const styles = StyleSheet.create({
  ...
  chipsArea:{paddingTop:7,paddingBottom:8,overflow:"visible",backgroundColor:"rgba(0,0,0,0.25)"},
