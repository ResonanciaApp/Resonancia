...
function Chip({ label, icon, sel, onPress }: { label: string; icon?: string; sel: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.chip, sel && styles.chipSel, { opacity: pressed ? 0.7 : 1 }]}>
      {sel && <View style={[StyleSheet.absoluteFill, { backgroundColor: "#DCA47A" }]} />}
    ...
    return (
      <>
        ...
        {activeTab === null && (
          ...
          >
            <Text style={{ fontFamily: "Manrope", fontSize: 15, fontWeight: "600", color: "#F7CB6B" }}>Todos los Sonidos Ancestrales</Text>
