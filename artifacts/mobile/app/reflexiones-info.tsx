...
const BENEFITS = [
  { icon: "book-open", label: "Integración interior",   sub: "Reflexionar sobre la experiencia consolida el aprendizaje emocional y espiritual." },
  { icon: "eye",       label: "Claridad de propósito",  sub: "El silencio contemplativo revela prioridades y disuelve la confusión." },
  { icon: "heart",     label: "Autocompasión",          sub: "La sabiduría no juzga — observa, comprende y transforma." },
  { icon: "sun",       label: "Presencia cotidiana",    sub: "Pequeñas dosis diarias de reflexión cambian la calidad de la atención." },
];
...
export default function ReflexionesInfoScreen() {
      ...
      <Animated.ScrollView showsVerticalScrollIndicator={false} scrollEventThrottle={16}
        ...
        <View style={{ height: H, overflow:"hidden" }}>
          ...
          <View style={[info.heroContent, { paddingBottom: insets.bottom + 48 }]}>
            <Text style={info.heroCat}>RESONANCIA · REFLEXIONES</Text>
            <Text style={info.heroTitle}>La Voz del Alma</Text>
            <Text style={info.heroSub}>
              Sabiduría, podcast, ASMR e historias — palabras que nutren el espíritu y aquietan el pensamiento.
            </Text>
        ...
        <Animated.View style={[info.section, { opacity: s2.opacity, transform:[{translateY:s2.translateY}] }]}>
          <Text style={info.sectionTitle}>El arte de la contemplación</Text>
          <Text style={info.sectionBody}>
            Reflexionar no es rumiar. Es crear un espacio entre el estímulo y la respuesta, entre
            el acontecimiento y el significado que le damos.{"\n\n"}
            Nuestras sesiones de Reflexiones combinan la profundidad filosófica de la sabiduría perenne
            con la accesibilidad del podcast contemporáneo.
          </Text>
        ...
        <Animated.View style={[info.section, { opacity: s3.opacity, transform:[{translateY:s3.translateY}] }]}>
          <Text style={info.sectionTitle}>Formatos disponibles</Text>
          {[
            { t:"Sabiduría",  d:"Reflexiones breves de tradiciones filosóficas y espirituales del mundo." },
            { t:"Podcast",    d:"Conversaciones profundas sobre psicología transpersonal, neurociencia y espiritualidad." },
            { t:"ASMR",       d:"Susurros, texturas sonoras y relatos que inducen estados de relajación profunda." },
            { t:"Historias",  d:"Relatos simbólicos y mitológicos que hablan al inconsciente." },
          ].map((g)=>(
        ...
        <Animated.View style={[info.section, { opacity: s4.opacity, transform:[{translateY:s4.translateY}] }]}>
          <Text style={info.sectionTitle}>¿Por qué reflexionar?</Text>
        ...
        <Animated.View style={[info.ctaSection, { opacity: s5.opacity, transform:[{translateY:s5.translateY}] }]}>
          <Text style={info.ctaTitle}>Un momento para ti</Text>
          <Text style={info.ctaBody}>Elige un formato y concédete unos minutos de contemplación.</Text>
          <Pressable onPress={()=>router.back()} style={info.ctaBtn}>
            ...
            <Text style={info.ctaBtnText}>Explorar Reflexiones</Text>
          </Pressable>
