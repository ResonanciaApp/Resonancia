        ...
        <View style={styles.topPanel}>
          ...
          <View style={{ height: 210 + topPad, width: "100%", pointerEvents: "none" }}>
            <View style={[styles.heroTextWrap, { paddingTop: topPad + 6, transform: [{ translateY: -35 }] }]}>
              <Text style={styles.heroTitle}>Mezclador 2</Text>
            </View>
          </View>
        ...
        {([ ["V1","musica"], ["V2","musica2"], ["V3","musica3"], ["V4","musica4"] ] as [string,string][]).map(([label, route]) => (
          <Pressable key={route} onPress={() => router.replace(`/(tabs)/${route}` as any)}
            style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999,
              backgroundColor: route === "musica2" ? "rgba(157,78,221,0.8)" : "rgba(0,0,0,0.65)",
              borderWidth: 1, borderColor: route === "musica2" ? "#9D4EDD" : "rgba(255,255,255,0.25)" }}>
