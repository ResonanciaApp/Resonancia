...
function Btn({ playing }: { playing: boolean }) {
  const glowStyle: React.CSSProperties = playing
    ? { filter: "drop-shadow(0 0 3px rgba(255,255,255,0.30)) drop-shadow(0 0 6px rgba(255,255,255,0.12))" }
    : {};
  return (
    <div
      style={{
    ...
    >
      <div style={glowStyle}>
        <Volume2
          size={15}
          color={playing ? "rgba(255,255,255,0.85)" : MUTED}
          strokeWidth={1.8}
        />
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "rgba(255,255,255,0.18)", fontSize: 10, margin: 0, maxWidth: 200, textAlign: "center", lineHeight: 1.6 }}>
        Refinado y minimalista.<br />Apenas separa del trazo.
      </p>
    </div>
  );
}
