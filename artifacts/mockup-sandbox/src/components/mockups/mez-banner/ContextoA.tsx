          ...
          {TABS.map(t => {
            ...
            return (
              <button key={t} onClick={() => setSel(t)} style={{
                ...
                padding:"6px 12px", borderRadius:999, border: active?"none":"1px solid #DEDEDE",
                background: active ? `linear-gradient(to bottom, ${gc[0]}, ${gc[1]})` : "#F5F4F2",
                color: active?"#fff":"rgba(0,0,0,0.6)", fontSize:12, fontWeight: active?700:400, cursor:"pointer",
                height:38,
              }}>{t}</button>
            );
          })}
        </div>
        {/* BANNER A — Borde sutil */}
        <div style={{
          display:"flex", alignItems:"center", justifyContent:"space-between", gap:10,
          border:"1px solid rgba(255,255,255,0.08)", borderRadius:12,
          background:"rgba(255,255,255,0.03)", padding:"9px 14px", margin:"0 0 12px",
        }}>
          <p style={{ margin:0, flex:1, fontSize:12, color:`rgba(255,255,255,${vis?0.9:0})`, transition:"opacity 0.45s ease", lineHeight:1.4 }}>
            {PLACEHOLDERS[idx]}
          </p>
          <button style={{ flexShrink:0, fontSize:10, fontWeight:600, color:"#D4AF37", background:"rgba(212,175,55,0.10)", border:"1px solid rgba(212,175,55,0.22)", borderRadius:20, padding:"5px 10px", cursor:"pointer", whiteSpace:"nowrap" }}>
            ¿Cómo te sientes?
          </button>
        </div>
      </div>
      <div style={{ padding:"16px", color:"rgba(255,255,255,0.15)", fontSize:10, textAlign:"center" }}>A — Borde sutil • Toca los tabs para ver el cambio de fondo</div>
    </div>
  );
}
