export function TabVarA() {
  const BG = "#1B060F";
  const WINE_SEL = "linear-gradient(180deg,#7A1515,#4A0C0C)";
  ...
  return (
    <div style={{ width: 390, height: 310, background: BG, overflow: "hidden", position: "relative", fontFamily: "-apple-system,BlinkMacSystemFont,sans-serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", gap: 8, paddingLeft: 15, paddingRight: 15, marginBottom: 12 }}>
        {tabs.map((t, i) => (
          <div key={i} style={{
            ...
            background: t.sel ? "linear-gradient(180deg,#7A1515,#4A0C0C)" : "rgba(255,255,255,0.12)",
            border: t.sel ? "none" : "1px solid rgba(255,255,255,0.20)",
            boxShadow: t.sel ? "0 0 14px 0 rgba(122,21,21,0.55)" : "none",
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5,
          }}>
            <span style={{ color: t.sel ? "#F4DAD5" : "rgba(255,255,255,0.65)", display: "flex" }}>
              {t.icon}
            </span>
            <span style={{ fontSize: 11, color: "#EDDFD5", letterSpacing: 0.1 }}>{t.label}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1, textAlign: "center", color: "rgba(244,218,213,0.40)", fontSize: 11, marginTop: 4 }}>
        A — Glass Blanco · bg rgba(255,255,255,0.12) + borde 0.20
      </div>
    </div>
  );
}
