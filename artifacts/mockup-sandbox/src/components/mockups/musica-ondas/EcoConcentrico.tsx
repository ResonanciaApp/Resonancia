...
const RING_DELAYS = [0, 0.26, 0.52];

export default function EcoConcentrico() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-6"
      ...
      <p style={{ color: GOLD, fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", opacity: 0.8 }}>
        Eco Concéntrico · 3 anillos en eco
      </p>
        ...
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              ...
              {/* 3 anillos concéntricos con retraso creciente */}
              <AnimatePresence>
                {(ripples[tab.id] ?? []).map(r =>
                  RING_DELAYS.map((delay, i) => (
                    <motion.div
                      key={`${r.id}-ring${i}`}
                      initial={{ scale: 0.9, opacity: 0.5 - i * 0.12 }}
                      animate={{ scale: 1.9 + i * 0.25, opacity: 0 }}
                      exit={{ opacity: 0 }}
                      transition={{
                        duration: 1.1,
                        delay,
                      ...
                      style={{
                        ...
                        inset: 0,
                        borderRadius: 16,
                        border: `${1.5 - i * 0.3}px solid ${GOLD}`,
                        pointerEvents: "none",
                      }}
                    />
                  ))
