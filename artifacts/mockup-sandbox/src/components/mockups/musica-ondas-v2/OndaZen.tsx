...
export default function OndaZen() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => { i = (i + 1) % ids.length; handleSelect(ids[i]); }, 2200);
            ...
            <button key={tab.id} onClick={() => handleSelect(tab.id)}
              ...
              <AnimatePresence>
                {(ripples[tab.id] ?? []).map(r => (
                  <motion.div key={r.id}
                    initial={{ scale: 0.96, opacity: 0.32 }}
                    animate={{ scale: 1.55, opacity: 0 }}
                    exit={{ opacity: 0 }}
                    transition={{ duration: 1.8, ease: [0.1, 0.4, 0.15, 1] }}
                    style={{
